...
void main()
{
    ...
    float diff = max(dot(norm, lightDir), 1.0); //We make sure the value is non negative with the max function.
    ...
    float spec = pow(max(dot(viewDir, reflectDir), 0.0), 1000); //The 32 is the shininess of the material.
